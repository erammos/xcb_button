/** @file gui.c
 *  @brief Demo - An absraction layer for XCB.
 *
 *  Implementations of very few gui widgets (Button, Text) using Xorg
 * programming
 *
 * @bug Quits if the '6x13' font, not found in the OS
...
static inline int
check_request (xcb_connection_t *c, xcb_void_cookie_t cookie, const char *msg)
{
...
static xcb_gc_t
gc_font_get (xcb_connection_t *c, xcb_screen_t *screen, xcb_window_t window,
...
static void
text_draw (xcb_connection_t *c, xcb_screen_t *screen, xcb_window_t window,
           int16_t x1, int16_t y1, const char *fontName, const char *label)
{
  xcb_void_cookie_t cookie_gc;
  ...
  length = strlen (label);
  gc = gc_font_get (c, screen, window, fontName);
  cookie_text
      = xcb_image_text_8_checked (c, length, window, gc, x1, y1, label);
...
static void
button_draw (gui_window *ptr, int16_t x, int16_t y, int16_t width,
             int16_t height, const char *label)
{
  ...
  text_draw (ptr->connection, ptr->screen, ptr->window,
             x + (width - text_width) / 2, y + height / 2, "6x13", label);
}

void
gui_dispose_window (void *p)
{
  gui_window *ptr = p;
...
gui_window *
gui_create_window (uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
...
bool
gui_button (void *p, uint32_t x, uint32_t y, uint32_t width, uint32_t height,
            const char *text)
{
  gui_window *ptr = p;
  if (ptr->event)
    {
      switch (ptr->event->response_type & ~0x80)
        ...
        case XCB_BUTTON_RELEASE:
          {
            xcb_clear_area (ptr->connection, 1, ptr->window, x, y, width,
                            height);
            return false;
            break;
          }
        case XCB_BUTTON_PRESS:
          {
            ...
            if (ev->event_x >= (x) && ev->event_x <= (x + width)
                && ev->event_y >= (y) && ev->event_y <= (y + height))
              {
                xcb_clear_area (ptr->connection, 0, ptr->window, x, y, width,
                                height);
...
void
gui_text (void *p, uint32_t x, uint32_t y, const char *font, const char *text)
{
  gui_window *ptr = p;
  text_draw (ptr->connection, ptr->screen, ptr->window, x, y, font, text);
}
